import { useState } from 'react';
import { Link } from 'react-router-dom';
import { useMutation, useQuery, useQueryClient } from '@tanstack/react-query';
import {
  Alert,
  Button,
  Card,
  EmptyState,
  Field,
  Icon,
  Input,
  Modal,
  PageHeader,
  Pagination,
  SkeletonRows,
  Textarea,
} from '@/components/ui';
import { toast } from '@/components/layout/Toast';
import { toApiError } from '@/lib/api';
import { authorApi } from '@/lib/services';
import { useAuth } from '@/store/auth';
import type { Author } from '@/types';

const EMPTY = { name: '', nationality: '', biography: '' };

/**
 * Author Management — DFD Level-0 module, backed by the Level-1
 * "List of Authors" data store.
 */
export default function AuthorsPage() {
  const queryClient = useQueryClient();
  const can = useAuth((s) => s.can);

  const [page, setPage] = useState(1);
  const [term, setTerm] = useState('');
  const [creating, setCreating] = useState(false);
  const [editing, setEditing] = useState<Author | null>(null);
  const [deleting, setDeleting] = useState<Author | null>(null);
  const [form, setForm] = useState(EMPTY);
  const [errors, setErrors] = useState<Record<string, string[]>>({});
  const [formError, setFormError] = useState<string | null>(null);

  const { data, isLoading } = useQuery({
    queryKey: ['authors', page, term],
    queryFn: () => authorApi.list({ page, per_page: 25, q: term || undefined }),
  });

  const refresh = () => {
    void queryClient.invalidateQueries({ queryKey: ['authors'] });
    void queryClient.invalidateQueries({ queryKey: ['authors-all'] });
  };

  const closeForm = () => {
    setCreating(false);
    setEditing(null);
    setForm(EMPTY);
    setErrors({});
    setFormError(null);
  };

  const save = useMutation({
    mutationFn: () =>
      editing ? authorApi.update(editing.author_id, form) : authorApi.create(form),
    onSuccess: () => {
      toast.success(editing ? 'Author updated.' : 'Author added.');
      closeForm();
      refresh();
    },
    onError: (error) => {
      const apiError = toApiError(error);
      setErrors(apiError.errors);
      setFormError(apiError.message);
    },
  });

  const remove = useMutation({
    mutationFn: (author: Author) => authorApi.remove(author.author_id),
    onSuccess: () => {
      toast.success('Author deleted.');
      setDeleting(null);
      refresh();
    },
    onError: (error) => {
      toast.error(toApiError(error).message);
      setDeleting(null);
    },
  });

  const openEdit = (author: Author) => {
    setEditing(author);
    setForm({
      name: author.name,
      nationality: author.nationality ?? '',
      biography: author.biography ?? '',
    });
  };

  const rows = data?.data ?? [];
  const meta = data?.meta;
  const err = (key: string) => errors[key]?.[0];

  return (
    <div className="space-y-6">
      <PageHeader
        title="Authors"
        subtitle="The library's list of authors. Every book is attributed to one of these records."
        action={
          can('author.manage') && (
            <Button icon="person_add" onClick={() => setCreating(true)}>
              Add author
            </Button>
          )
        }
      />

      <Card padded={false}>
        <div className="border-b border-outline-variant p-4">
          <div className="relative max-w-md">
            <Icon
              name="search"
              className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-[18px] text-on-surface-variant"
            />
            <Input
              value={term}
              onChange={(e) => {
                setTerm(e.target.value);
                setPage(1);
              }}
              placeholder="Search authors by name…"
              className="pl-10"
            />
          </div>
        </div>

        {isLoading ? (
          <SkeletonRows rows={6} cols={4} />
        ) : rows.length === 0 ? (
          <EmptyState
            icon="groups"
            title="No authors found"
            description="Authors are created here, then selected when cataloguing a book."
            action={
              can('author.manage') && (
                <Button icon="person_add" onClick={() => setCreating(true)}>
                  Add author
                </Button>
              )
            }
          />
        ) : (
          <>
            <div className="overflow-x-auto">
              <table className="w-full text-body-md">
                <thead>
                  <tr className="border-b border-outline-variant bg-surface-container-low text-left">
                    <th className="px-4 py-3 text-label-md uppercase text-on-surface-variant">Name</th>
                    <th className="px-4 py-3 text-label-md uppercase text-on-surface-variant">
                      Nationality
                    </th>
                    <th className="px-4 py-3 text-label-md uppercase text-on-surface-variant">
                      Biography
                    </th>
                    <th className="px-4 py-3 text-right text-label-md uppercase text-on-surface-variant">
                      Books
                    </th>
                    <th className="px-4 py-3 text-right text-label-md uppercase text-on-surface-variant">
                      Actions
                    </th>
                  </tr>
                </thead>
                <tbody>
                  {rows.map((author) => (
                    <tr
                      key={author.author_id}
                      className="border-b border-surface-container last:border-0 hover:bg-surface-container-low"
                    >
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-secondary-container text-on-secondary-container">
                            <Icon name="person" className="text-[18px]" />
                          </span>
                          <span className="font-medium text-on-surface">{author.name}</span>
                        </div>
                      </td>
                      <td className="px-4 py-3 text-on-surface-variant">
                        {author.nationality ?? '—'}
                      </td>
                      <td className="max-w-md px-4 py-3">
                        <span className="line-clamp-1 text-body-sm text-on-surface-variant">
                          {author.biography ?? '—'}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-right tabular">
                        <Link
                          to={`/search?author_id=${author.author_id}`}
                          className="font-semibold text-primary hover:underline"
                        >
                          {author.books_count}
                        </Link>
                      </td>
                      <td className="px-4 py-3">
                        {can('author.manage') && (
                          <div className="flex justify-end gap-1">
                            <Button
                              variant="ghost"
                              size="sm"
                              icon="edit"
                              onClick={() => openEdit(author)}
                              aria-label={`Edit ${author.name}`}
                            />
                            <Button
                              variant="ghost"
                              size="sm"
                              icon="delete"
                              onClick={() => setDeleting(author)}
                              aria-label={`Delete ${author.name}`}
                            />
                          </div>
                        )}
                      </td>
                    </tr>
                  ))}
                </tbody>
              </table>
            </div>

            {meta && (
              <Pagination
                page={meta.current_page}
                lastPage={meta.last_page}
                total={meta.total}
                from={meta.from}
                to={meta.to}
                onChange={setPage}
              />
            )}
          </>
        )}
      </Card>

      <Modal
        open={creating || Boolean(editing)}
        onClose={closeForm}
        title={editing ? 'Edit author' : 'Add an author'}
        footer={
          <>
            <Button variant="ghost" onClick={closeForm}>
              Cancel
            </Button>
            <Button icon="save" loading={save.isPending} onClick={() => save.mutate()}>
              {editing ? 'Save changes' : 'Add author'}
            </Button>
          </>
        }
      >
        <div className="space-y-4">
          {formError && <Alert tone="danger">{formError}</Alert>}

          <Field label="Full name" htmlFor="name" required error={err('name')}>
            <Input
              id="name"
              value={form.name}
              onChange={(e) => setForm({ ...form, name: e.target.value })}
              placeholder="Abraham Silberschatz"
              autoFocus
              invalid={Boolean(err('name'))}
            />
          </Field>

          <Field label="Nationality" htmlFor="nationality" error={err('nationality')}>
            <Input
              id="nationality"
              value={form.nationality}
              onChange={(e) => setForm({ ...form, nationality: e.target.value })}
              placeholder="American"
            />
          </Field>

          <Field label="Biography" htmlFor="biography" error={err('biography')}>
            <Textarea
              id="biography"
              value={form.biography}
              onChange={(e) => setForm({ ...form, biography: e.target.value })}
              placeholder="A short note about the author…"
            />
          </Field>
        </div>
      </Modal>

      <Modal
        open={Boolean(deleting)}
        onClose={() => setDeleting(null)}
        title="Delete this author?"
        footer={
          <>
            <Button variant="ghost" onClick={() => setDeleting(null)}>
              Cancel
            </Button>
            <Button
              variant="danger"
              icon="delete"
              loading={remove.isPending}
              onClick={() => deleting && remove.mutate(deleting)}
            >
              Delete author
            </Button>
          </>
        }
      >
        {deleting && (
          <div className="space-y-3">
            <p className="text-body-md text-on-surface">
              Delete <span className="font-semibold">“{deleting.name}”</span>?
            </p>
            {deleting.books_count > 0 && (
              <Alert tone="warning" title="This author has books in the catalog">
                <span className="font-semibold">{deleting.books_count}</span> book(s) are attributed
                to them. Reassign those books to another author first — the system will refuse the
                delete otherwise.
              </Alert>
            )}
          </div>
        )}
      </Modal>
    </div>
  );
}
